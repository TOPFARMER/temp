...
Public Class Ledger
    ...
    '基块的信息
    Public Shared TMP_HASH As String = "000000000000000000000000"
    Public Shared BASE_HASH As String = "0008NtOX5fpuSX21PjSrZw=="
    Public Shared BASE_NONCE As String = "0555DD"
    ...
    Public blockchain As New List(Of Block) '存放区块链
    ...
    Public Sub New(ByVal id As Integer)
        ...
        ElseIf mId < mChainStorge.Count Then
    ...
    Public Sub GetAndCheckMessage(ByVal source As Integer, ByVal code As String)
        ...
        Else
            ...
            If  code.split("|")(0) = "IsBlk" Then '若信息为区块
                Dim blk As Block = CheckBlock(code.SubString(6)) '区块进行组装并检查
    ...
    Public Function CheckBlock(ByVal str As String) As Block 
        ...
        If blk.PrevHash <> blockchain(blockchain.Count - 1).MyHash Then '收到的块的上一个哈希对不齐，丢弃
            Return Nothing
        End If
        For Each code As string In blk.Messages '检查内部记录是否符合签名，不符合则丢弃
            If Left(code.split("|")(1), 3) = "mId" Then '检查末尾
                ...
                If msg.split("|")(1) <> "mId" & CStr(i) Then '解出信息无意义,丢弃
                    Return Nothing
                End If
            End If
        Next
        If blk.MyHash <> GetHash(blk.BlockId, blk.Nonce, blk.Messages, blk.PrevHash) Then '哈希对不上，丢弃
            Return Nothing
        End If
        Return blk
    End Function
    ...
    Public Function GenerateBlock(ByVal hash_nonce As String) As Block
        ...
        Dim nonce As String = hash_nonce.split("|")(1)
        Dim prev_hash = blockchain(blockchain.Count - 1).MyHash 
        Dim my_hash As String = hash_nonce.split("|")(0)
        Return New Block(bId, nonce, msg_buff, prev_hash, my_hash)
    End Function
  ...
  ' End Function
    ...
    '与foxtable的接口:获取最新区块
    Public Shared Function GetNewBlock(ByVal id As Integer) As Block
        Return mChainStorge(id)(mChainStorge(id).Count - 1)
    End Function

    '与foxtable的接口:确认签名信息，得出内容
    Public Shared Function GetDecryptMsg(ByVal code As String) As String()
        If Left(code.split("|")(1), 3) = "mId" Then '检查末尾
                Dim i As Integer = CInt(Right(code.split("|")(1), 1)) '得到信息来源主机号
                Dim msg As String = RSA.decryptByPublic(code.split("|")(0) ,mRSABank(i).public_key) '对密文使用公钥解密
                If msg.split("|")(1) <> "mId" & CStr(i) Then '解出信息无意义,丢弃
                    Return Nothing
                End If
                Dim messages() As String = {msg.split("|")(0), Cstr(i), mRSABank(i).public_key.e & "|" & mRSABank(i).public_key.N}
                Return messages
        Else
            Return Nothing
        End If
...
Public Class Block
    ...
    '构造区块
    Public Sub New(ByVal bId As Integer, ByVal nce As String, ByVal msgs As List(Of String), ByVal prev_hash As String, ByVal my_hash As String)
        BlockId = bId
        Nonce = nce
    ...
    Public Function Block2Msg() As String
        Dim msgs As String = ""
        For Each code As String In Messages
            msgs = msgs & code & "\"
        Next
