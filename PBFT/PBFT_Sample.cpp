...
// 每个进程都有一组关于每个结点传来信息的map，这个map结构的index值是一个路径
// ——一个信息所经过的进程结点的ID序列string。每个接收到的信息结点的输入值在算法
// 的第一轮（互发信息轮）被设置好，它们的输出值则在算法的第二轮（统计数量最多的值）
// 得出。
//
...
// in a map
// 要注意的是尽管我们在算法中不会用到默认构造函数，但我们仍然需要有这个函数因为
// 我们将会把它存储在一个map组里
struct Node {
...
// and so on.
// 特征基类用于定义进程的消息传递行为。（Traits可能不是最好的名称，因为我稍微重载了它的
// 通常用法，但是它很接近。）Process类有一个全局对象，它使用Traits类型来获取用于
// 描述谁是将军、谁是副官等信息的所有信息。
//
...
// class
// Process类有一个静态Traits成员，它的方法在传信和决策阶段的关键时刻被调用，以确定如何操作。
// 你可以通过修改特征类来修改整个算法运行的配置。
...
// 这个算法实现了一个发错信息的将军和发错信息的进程2。将军将会发一或零给所有的进程
// 而进程二不管它接受到什么也即是它本应发送什么，它都会发一给每一个进程。
...
public :
    ...
    // in its root node.
    // 这个函数会返回源节点本身的真值。源结点可能会发送错误的值给其他的进程，但是函数返回的
    // Node值都会是它的根节点。
    ...
    // matter.
    // 在这种情况下，我们算法设定将军的输入值是0，这不就得到我们想要的共识值了。的确是这样，
    // 但是既然将军是乱发信息的结点，所以这个0并不意味着共识值，这个值其实没什么意义。
    Node GetSourceValue() {
